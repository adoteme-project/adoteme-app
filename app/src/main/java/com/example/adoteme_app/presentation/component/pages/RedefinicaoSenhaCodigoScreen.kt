...
@Composable
fun RedefinicaoSenhaCodigoScreen(
    email: String,
    navController: NavController,
   viewModel: RedefinicaoOptViewModel = koinViewModel()
) {

    val codigo = viewModel.codigo
    ...
    Scaffold(
        ...
        Box(
            ...
            Column(
                ...
                Text(text = "Código", modifier = Modifier.padding(bottom = 4.dp))
                OtpInput{novoCodigo -> viewModel.onCodigoChange(novoCodigo)}
                if(erro == "Erro ao validar o código") {
                    Text(erro, color = Color.Red)
                }
                Spacer(modifier = Modifier.height(21.dp))
                Button(
                    onClick = {
                        viewModel.validarCodigo(email) {
                            navController.navigate("nova_senha/${email}")
                        }
                    },
                    enabled = !isLoading,
                    colors = ButtonColors(
